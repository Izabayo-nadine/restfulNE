...
export const inventoryReport = asyncHandler(async (req, res) => {
  const period = req.query.period || getDefaultInventoryPeriod();
  ...
  res.json({
    success: true,
    data: { period, total, byType, byStatus, recent, generatedAt: new Date() },
  });
});
...
export const maintenanceReport = asyncHandler(async (req, res) => {
  ...
  const [history, total, frequency, recent] = await Promise.all([
    ...
    Maintenance.find().sort("-createdAt").limit(getReportRecentLimit()),
  ]);
  res.json({
    success: true,
    data: {
...
function drawPdfTable(doc, headers, rows, startY = 120) {
  ...
  headers.forEach((h, i) => {
    doc.text(String(h), left + i * colWidth, y, {
      width: colWidth - 4,
      ellipsis: true,
    });
  });
...
export const exportReport = asyncHandler(async (req, res) => {
  ...
  if (
    !["inventory", "inspection", "compliance", "maintenance"].includes(type)
  ) {
    return res
      .status(400)
  ...
  if (!["csv", "pdf"].includes(format)) {
    return res
      .status(400)
      .json({ success: false, message: "Format must be csv or pdf" });
  }
  ...
  if (type === "inventory") {
    ...
    const now = new Date();
    const warningEnd = new Date(
      now.getTime() + getExpiryWarningDays() * 86400000,
    );
